      ...
      <table className="min-w-full text-sm">
        ...
        <tbody>
          {rows.length === 0 ? (
            ...
            rows.map((row, idx) => (
              <tr key={`${row.Product_ID}-${row.BatchNumber}-${idx}`} className="border-t border-gray-100 italic-row hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3 font-medium text-gray-900">{row.productname || "-"}</td>
                <td className="px-4 py-3 text-gray-600">{row.generic_name || "-"}</td>
                <td className="px-4 py-3">{row.BatchNumber || "-"}</td>
                <td className="px-4 py-3">{formatDate(row.expiry_date)}</td>
                <td className="px-4 py-3 text-gray-600">{row.AgencyName || "-"}</td>
                ...
                <td className="px-4 py-3 text-right font-semibold text-[#3F75B0]">{formatMoney(row.total_sales_value)}</td>
                <td className="px-4 py-3 text-xs">{formatDate(row.last_sale_date)}</td>
              </tr>
            ))
          )}
        </tbody>
